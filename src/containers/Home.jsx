...
const Home = () => {
  ...
  return (
    <>
      ...
      <Tercera><TituloCont><ProductosA>Productos agrícolas</ProductosA><Image src="https://res.cloudinary.com/dcsn54xoj/image/upload/v1658099454/MiCampo/Vector_lgqq8t.png" alt="" /></TituloCont>
        <Linea>{prod6.map((element, index) =>
          <ContImagen key={index} as={Link} to={`/detalle/${element.nombre}`}> <ProdImg src={element.img} alt="" /><div><p>{element.nombre}<br />Localización </p></div></ContImagen>)}</Linea>
        <BotVerde onClick={() =>
          navigate(

  ...
  return (
    <>
     ...
     <Search1/> <App/> </Search>
      ...
      <Linea>{agro6.map((element,index)=>(<Agroin key={index}><img src={element.img} alt="" /><Negro> <Ali>{element.nombre} <br /> <Spali><GoLocation />{element.ubi}</Spali></Ali> <Enter>Ver <BsArrowRightShort /></Enter></Negro></Agroin>))}</Linea>
      <BotVerde onClick={() =>
                                 navigate(

...
const Aliados = () => {
  ...
  return (
    ...
    <Cuarta><TituloCont><ProductosA>Nuestros Agricultores</ProductosA><Image src="https://res.cloudinary.com/dcsn54xoj/image/upload/v1658259882/MiCampo/Group_anru90.png" alt="" /></TituloCont><Linea>
    {agricultores.map((element, index)=>(<TarjetaAliado key={index}><Aliado src={element.data.photoURL} alt="" /><Negro> <Ali> {element.data.name}<br /> <Spali><GoLocation />{element.data.ubication}</Spali></Ali> <Enter>Contactar Agricultor <BsArrowRightShort /></Enter></Negro></TarjetaAliado>))}</Linea></Cuarta>
